...
The Python SDK must be installed in the selected interpreter; the driver does not add the checkout to its import path. For the final AWS gate, use the isolated interpreter from the verified release wheel with Python's `-I` flag, pass the exact downloaded native executable through `--binary`, and verify the imported SDK module locations and bytes against that wheel. Record this installation proof with the measured binary hash. Local diagnostics may use a development SDK installation, but that scope must remain explicit.
