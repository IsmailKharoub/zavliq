...
Before either original runtime starts, verify checks both unchanged identity files and confirms B still has none of the historical events. It durably records this condition before the first restore sync. The original recipient must decrypt the old text and exact JSON, inspect the redacted encrypted-file marker, and download the old file into a fresh path whose SHA-256 matches the source. Full inbox output exposes `file: {url, encrypted: true}` for encrypted media; keys and the full Matrix descriptor remain in the private native store. The fixture refuses plaintext URLs or exposed file keys. Native attachment download uses the private descriptor and bypasses its media cache. Both original devices must then exchange new encrypted text and JSON using their existing trust and crypto state, without pairing, key import or re-verification.
