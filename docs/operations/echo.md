...
The Docker image, profile wiring and Echo-aware backup/restore changes have source validation only until the deployment verification records a real container build and restore drill. The earlier native Echo checks do not substitute for those operations checks.
