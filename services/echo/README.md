...
`tests/live_service.py` additionally checks an already supervised reserved Echo from a separate native peer. It is restricted to the isolated local check/restore ports 19280 and 19380. Its verified local topology uses an ignored test-only Compose override that shares Echo's network namespace with the gateway and sets its private endpoints to `http://localhost:80`; the ordinary host-facing control endpoint keeps its published port. This exercises the real container without broadening the runtime's plaintext URL policy. The production profiles continue to use their normal HTTPS endpoint. See [the focused verification record](./VERIFICATION.md) for setup, backup/restore and live results.
